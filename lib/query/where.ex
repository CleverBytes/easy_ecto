      ...
      defp query_where(queryable, {k, map_cond}, opts) when is_map(map_cond) do
        ...
        Enum.reduce(map_cond, queryable, fn {key, value}, queryable ->
          case key do
            "$like" ->
              if opts[:binding] == :last do
                from([..., c] in queryable, where: like(field(c, ^String.to_existing_atom(k)), ^value))
              else
            ...
            "$ilike" ->
              if opts[:binding] == :last do
                from([..., c] in queryable, where: ilike(field(c, ^String.to_existing_atom(k)), ^value))
              else
            ...
            "$notlike" ->
              if opts[:binding] == :last do
                from([..., c] in queryable, where: not like(field(c, ^String.to_existing_atom(k)), ^value))
              else
            ...
            "$lt" ->
              if opts[:binding] == :last do
                if Query.Helper.is_field?(value) do
                  ...
                  from(
                    [..., c] in queryable,
                    where: field(c, ^String.to_existing_atom(k)) < field(c, ^String.to_existing_atom(value))
                  )
                else
                ...
                if Query.Helper.is_field?(value) do
                  ...
                  from(
                    q in queryable,
                    where: field(q, ^String.to_existing_atom(k)) < field(q, ^String.to_existing_atom(value))
                  )
                else
            ...
            "$notin" ->
              if opts[:binding] == :last do
                from([..., c] in queryable, where: field(c, ^String.to_existing_atom(k)) not in ^value)
              else
